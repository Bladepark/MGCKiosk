    ...
    if (coffeeList.isNotEmpty()) {
        when (selectCoffee?.toIntOrNull()) {
            in 1..coffeeList.size -> {
                ...
                val addList = AllMegaMenu(
                    selectedCoffee.name,
                    selectedCoffee.price,
                    selectedCoffee.time,
                    selectedCoffee.termo
                )
    ...
    if (adeJuiceList.isNotEmpty()) {
        when (selectAdeJuice?.toIntOrNull()) {
            in 1..adeJuiceList.size -> {
                ...
                val addList = AllMegaMenu(
                    selectedAdeJuice.name,
                    selectedAdeJuice.price,
                    selectedAdeJuice.time,
                    selectedAdeJuice.termo
                )
                jangBaGuNiList.add(addList)
                println("장바구니에 추가되었습니다\n")
                selectAdeJuiceMenu()
        ...
        when (selectDessert?.toIntOrNull()) {
            in 1..dessertList.size -> {
                ...
                val addList = AllMegaMenu(
                    ...
                    selectedDessert.time,
                    selectedDessert.termo
                )
                jangBaGuNiList.add(addList)
                println("장바구니에 추가되었습니다\n")
                selectDessertMenu()
...
//ICE/HOT 기능은 제품 선택 후 coffee.termo 에서 값이 있으면 선택할 수 있도록
//제품 선택 후 펑션 (COMMENT! 이용불가 안내를 메뉴판에서 구현해서 메뉴가 없어요 기능 수정이 필요함)
fun coffeeMenu() {
    val coffeeList: List<Coffee> = menuList.filterIsInstance<Coffee>()
...
//2023.12.07 ~16:00까지 기능 통합 시도하다 코드가 꼬여 키오스크 3으로 롤백, 진짜 개짜증난다 ㅅ1발
//롤백하고 런 테스트 하니까 데이터 위치가 같은 곳에서 불러온다던가 하는 문제가 있어 모두 수정함
//일단 데이터 리스트 장바구니 추가 기능부터 구현해 본다 -> 17:10 장바구니 기능 중 리스트업 완료, 장바구니 삭제 기능과 주문 기능 추가 해야함

/* 기능 분리 다른 방법으로 해야 할 것 같다
fun jangListView(){
    println("\n[     장 바 구 니     ]\n")
    if (jangBaGuNiList.isNotEmpty()){
        var finalPrice = 0
